...
```mermaid
flowchart TD
    A[ChEMBL EGFR bioactivity<br>~1253 records] --> B[Data cleaning<br>standardization, dedup]
    B --> C1[Model 1: EGFR backbone<br>XGBoost/RF, 5-seed scaffold split]
    B --> C2[Model 2: WT-proxy comparator<br>XGBoost, 5-seed scaffold split]
    B --> C3[Model 3: L858R calibration<br>LOOCV n=22, EXPLORATORY]
    C1 & C2 --> D[Model 4: Derived selectivity<br>n=9 pairs, not significant]
    B --> E[Fingerprint ablation<br>6 FP types x 3 models x 5 seeds]
    B --> F[GNN benchmark<br>GINEConv x4, QSAR wins]
    C1 --> G[Top-50 candidates<br>backbone predicted pIC50]
    G --> H1[Phase B1: Structure prep<br>2ITZ L858R, 2ITY WT, Biopython+pdbfixer]
    H1 --> H2[Phase B2: Vina docking<br>sanity check PASS, library 49/50 docked]
    H2 --> H3[Phase B3: GNINA CNN rescore<br>BORDERLINE, Vina retained]
    H2 --> I[Noise quantification<br>5 seeds x 15 cmpds, 6 confident hits]
    I --> J[ADMET filtering<br>Lipinski, Veber, PAINS, Brenk, QED, SA]
    B --> K[Two-stage GRU generator<br>MOSES pretrain, EGFR fine-tune, 92.3% validity]
    K --> L[RL fine-tuning<br>sigma=0.5 REWARD_HACKING, sigma=0.25 INCONCLUSIVE]
    K & G --> M[Applicability domain<br>max Tanimoto, 3 bands]
    G & K --> N[Final composite ranking v2<br>0.30 activity + 0.30 sel + 0.20 aff + 0.20 ADMET]
    N --> O[FastAPI backend<br>precomputed artifacts, no docking at request time]
    O --> P[Streamlit dashboard<br>7 pages, API with local fallback]
    P --> Q[Docker packaging<br>api + dashboard, models bind-mounted]
    P --> R[Discovery Copilot page<br>deterministic tool orchestrator, no LLM<br>grounded answer, evidence, warnings]
```
