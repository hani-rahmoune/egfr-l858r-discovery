...
from src.data.cleaning import (
    add_canonical_smiles, deduplicate_by_smiles,
    filter_by_heavy_atom_count, is_valid_smiles,
    remove_invalid_smiles, run_full_cleaning_pipeline,
    standardize_smiles,
)
...
class TestIsValidSmiles:
    def test_valid(self): assert is_valid_smiles("CCO") is True
    def test_empty(self): assert is_valid_smiles("") is False
    def test_none(self): assert is_valid_smiles(None) is False
    def test_garbage(self): assert is_valid_smiles("not_smiles") is False
    def test_unbalanced(self): assert is_valid_smiles("C(C") is False


@pytest.mark.unit
class TestStandardizeSmiles:
    def test_returns_string(self): assert isinstance(standardize_smiles("CCO"), str)
    def test_invalid_none(self): assert standardize_smiles("bad") is None
    def test_none_none(self): assert standardize_smiles(None) is None
...
class TestDeduplicateBySmiles:
    def test_removes_dupes(self):
        df = pd.DataFrame({"canonical_smiles": ["CCO", "CCO", "c1ccccc1"], "pic50": [7.0, 7.5, 6.0]})
...
class TestFullPipeline:
    ...
    def test_output_leq_input(self, tiny_molecules_df):
        assert len(run_full_cleaning_pipeline(tiny_molecules_df)) <= len(tiny_molecules_df)