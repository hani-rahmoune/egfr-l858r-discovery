...
from src.data.standardization import (
    add_activity_label, convert_to_molar, ic50_to_pic50,
    remove_invalid_activity, run_activity_standardization,
)


@pytest.mark.unit
class TestConvertToMolar:
    def test_nm(self): assert convert_to_molar(100, "nM") == pytest.approx(1e-7)
    def test_um(self): assert convert_to_molar(1, "uM") == pytest.approx(1e-6)
    def test_unicode_um(self): assert convert_to_molar(1, "µM") == pytest.approx(1e-6)
    def test_unknown_none(self): assert convert_to_molar(100, "xyz") is None
    def test_case_insensitive(self): assert convert_to_molar(100, "NM") == pytest.approx(1e-7)


@pytest.mark.unit
class TestIc50ToPic50:
    def test_1nm_is_9(self): assert ic50_to_pic50(1e-9) == pytest.approx(9.0, abs=1e-5)
    def test_1um_is_6(self): assert ic50_to_pic50(1e-6) == pytest.approx(6.0, abs=1e-5)
    def test_zero_none(self): assert ic50_to_pic50(0) is None
    def test_negative_none(self): assert ic50_to_pic50(-1e-9) is None
    def test_nan_none(self): assert ic50_to_pic50(float("nan")) is None
